...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['accept_request'])) {
        ...
        $stmt = $con2->prepare("UPDATE request SET repair_status = 'Work In Progress', expected_delivery = ? WHERE id = ? AND mechanic_assign = ?");
        $stmt->bind_param("sis", $expected_delivery, $request_id, $mechanic['name']);
        $stmt->execute();
        $stmt->close();
    }
}

$stmt2 = $con2->prepare("SELECT * FROM request WHERE mechanic_assign = ? and repair_status = 'Mechanic Assigned' ");
        ...
        <div class="col">
            <div class="card mt-5">
                <div class="card-header">
                    <h2 class="display-8 text-center">Your Work Requests</h2>
                <div class="card-body">
                        ...
                        <?php if (count($requests) > 0): ?>
                            <?php foreach ($requests as $request): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($request['repair_status']); ?></td>
                                    <td><?php echo htmlspecialchars($request['assigned_date']); ?></td>
                                    <td>
                                        <input type="date" name="expected_delivery" id="date-<?php echo htmlspecialchars($request['id']); ?>" class="form-control" required oninput="checkDateInput(<?php echo htmlspecialchars($request['id']); ?>)">
                                    </td>
                                    <td>
                                        <form action="" method="post" id="form-<?php echo htmlspecialchars($request['id']); ?>">
                                            <input type="hidden" name="request_id" value="<?php echo htmlspecialchars($request['id']); ?>">
                                            <input type="hidden" name="expected_delivery" id="hidden-date-<?php echo htmlspecialchars($request['id']); ?>">
                                            <button type="submit" name="accept_request" id="accept-<?php echo htmlspecialchars($request['id']); ?>" class="btn btn-success" disabled onclick="setHiddenDate(<?php echo htmlspecialchars($request['id']); ?>)">Accept</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
...
<script>
    function checkDateInput(requestId) {
        const dateInput = document.getElementById(`date-${requestId}`);
        const acceptButton = document.getElementById(`accept-${requestId}`);
        acceptButton.disabled = !dateInput.value;
    }

    function setHiddenDate(requestId) {
        const dateInput = document.getElementById(`date-${requestId}`);
        const hiddenDateInput = document.getElementById(`hidden-date-${requestId}`);
