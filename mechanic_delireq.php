...
$query = "SELECT * FROM request WHERE request_status = 'sent To Mechanic'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['accept_request'])) {
        $update_query = "UPDATE request SET request_status = 'Accepted', reply = '-' WHERE id = ?";
        ...
        if ($stmt->execute()) {
            header('Location: mechanic_delireq.php');
        } else {
    ...
    if (isset($_POST['cancel_request'])) {
        $reply = $_POST['reply'];
        $update_query = "UPDATE request SET request_status = 'Cancelled', reply = ? WHERE id = ?";
        $stmt = $con->prepare($update_query);
        if ($stmt === false) {
            die("Prepare failed: " . htmlspecialchars($con->error));
        }
        $stmt->bind_param("si", $reply, $req_id);
        if ($stmt->execute()) {
            header('Location: mechanic_delireq.php');
        } else {
            echo "Error updating request: " . htmlspecialchars($stmt->error);
...
<head>
    ...
    <title>Mechanic View Requests</title>
    <style>
        body, html {
            margin: 0;
            display: flex;
...
<body class="gradient-background">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                                                <input type="hidden" name="req_id" value="<?php echo htmlspecialchars($row['id']); ?>">
                                        ...
                                        <td>
                                            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                                                ...
                                                <div class="form-group mt-2">
                                                    ...
                                                    <textarea name="reply" class="form-control" rows="3" oninput="checkReply(this)"></textarea>
                                                    <button type="submit" name="cancel_request" class="btn btn-danger" disabled>Cancel</button>
                                                </div>
                                            </form>
                                        </td>
                                    </tr>
    ...
    <script>
        function checkReply(textarea) {
            const cancelButton = textarea.closest('form').querySelector('button[name="cancel_request"]');
