...
if ($con1->connect_error) {
    ...
    $stmt1 = $con1->prepare("SELECT name FROM mechanic_info WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['accept_request'])) {
        ...
        $expected_delivery = date('yy-mm-dd');
        $bill_amount = $_POST['bill_amount'];

        $stmt = $con2->prepare("UPDATE request SET repair_status = 'Work Done', bill_amount = ?, expected_delivery = ? WHERE id = ? AND mechanic_assign = ?");
        $stmt->bind_param("ssis", $bill_amount, $expected_delivery, $request_id, $mechanic['name']);
        $stmt->execute();
        $stmt->close();
    }
    ...
    if (isset($_POST['update_delivery'])) {
        ...
        $stmt = $con2->prepare("UPDATE request SET expected_delivery = ? WHERE id = ? AND mechanic_assign = ?");
        $stmt->bind_param("sis", $new_delivery_date, $request_id, $mechanic['name']);
        $stmt->execute();
        $stmt->close();
    }
}

$stmt2 = $con2->prepare("SELECT * FROM request WHERE mechanic_assign = ? and repair_status = 'Work In Progress' and bill_amount = 'not updated'");
$stmt2->bind_param("s", $mechanic['name']);
$stmt2->execute();
        ...
        <div class="col">
            <div class="card mt-5">
                <div class="card-header">
                    <h2 class="display-8 text-center">Work Status</h2>
                </div>
                <div class="card-body">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="" method="post">
                                            ...
                                            <input type="date" name="new_delivery_date" class="form-control" value="<?php echo htmlspecialchars($request['expected_delivery']); ?>" required>
                                            <button type="submit" name="update_delivery" class="btn btn-primary mt-2">Update</button>
                                        </form>
                                    </td>
                                    <td>
                                        <input type="text" name="bill_amount" id="bill-amount-<?php echo htmlspecialchars($request['id']); ?>" class="form-control" required oninput="enableAcceptButton(<?php echo htmlspecialchars($request['id']); ?>)">
                                    </td>
                                    <td>
                                        <form action="" method="post">
                                            <input type="hidden" name="request_id" value="<?php echo htmlspecialchars($request['id']); ?>">
                                            <input type="hidden" name="expected_delivery" id="hidden-date-<?php echo htmlspecialchars($request['id']); ?>" value="<?php echo htmlspecialchars($request['expected_delivery']); ?>">
                                            <input type="hidden" name="bill_amount" id="hidden-bill-amount-<?php echo htmlspecialchars($request['id']); ?>">
                                            <button type="submit" name="accept_request" id="accept-<?php echo htmlspecialchars($request['id']); ?>" class="btn btn-success" disabled>Done</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
...
<script>
    function enableAcceptButton(requestId) {
        ...
        const expectedDeliveryInput = document.getElementById(`hidden-date-${requestId}`);
        
        if (billAmountInput.value.trim() !== "" && expectedDeliveryInput.value.trim() !== "") {
