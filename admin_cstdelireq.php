...
$query = "SELECT * FROM request WHERE request_status = 'sent To Admin'";
$result = mysqli_query($con, $query);

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $req_id = $_POST['req_id'];

    if (isset($_POST['forward_to_mechanic'])) {
        $update_query = "UPDATE request SET request_status = 'sent To Mechanic', reply = '-' WHERE id = ?";
        ...
        if ($stmt->execute()) {
            header('Location: admin_cstdelireq.php');
        } else {
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                                    ...
                                    <button type="submit" name="forward_to_mechanic" class="btn btn-primary">Forward to Mechanic</button>
                                    
                                    <td><div class="form-group mt-2">
                                        <label for="reply">Reply:</label>
                                        <textarea name="reply" class="form-control" rows="3"></textarea>
                                        <button type="submit" name="cancel_request" class="btn btn-danger">Cancel</button></td>
                                    </div></td>
                                </form>
                            </td>
                        </tr>
