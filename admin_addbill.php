...
// Fetch the bill amount based on the request ID
$request_id = $_GET['id']; // Assuming request ID is passed via URL parameter
$query = "SELECT bill_amount FROM request WHERE id = $request_id";
$result = mysqli_query($con, $query);
$row = mysqli_fetch_assoc($result);
$bill_amount = $row['bill_amount'];

// Handle form submission to update the total bill
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_bill'])) {
    $mechanic_charge = $_POST['mechanic_charge'];
    $total_bill = $bill_amount + $mechanic_charge;
    $update_query = "UPDATE request SET mechanic_charge = '$mechanic_charge', total_bill = '$total_bill' WHERE id = $request_id";
    mysqli_query($con, $update_query);

    // Redirect to a confirmation page or back to the list
    header("Location:admin_viewsts.php"); // Change this to the desired page
    exit();
}
?>
        ...
        <div class="col">
            <div class="card mt-5">
                <div class="card-header">
                    <h2 class="display-8 text-center">Update Bill</h2>
                </div>
                <div class="card-body">
                    <form method="POST" action="">
                        <div class="mb-3">
                            <label for="bill_amount" class="form-label">Bill Amount</label>
                            <input type="text" class="form-control" id="bill_amount" name="bill_amount" value="<?php echo $bill_amount; ?>" disabled>
                        </div>
                        <div class="mb-3">
                            <label for="mechanic_charge" class="form-label">Mechanic Charge</label>
                            <input type="number" class="form-control" id="mechanic_charge" name="mechanic_charge" required>
                        </div>
