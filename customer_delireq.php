...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request'])) {
    ...
    error_log("Request ID: $req_id");
    error_log("Request: $request");

    $update_query = "UPDATE request SET request = ? , request_status = 'sent To Admin' WHERE id = ?";
    $stmt = $con->prepare($update_query);
    if ($stmt === false) {
        die("Prepare failed: " . htmlspecialchars($con->error));
    }
    $stmt->bind_param("si", $request, $req_id);
    if ($stmt->execute()) {
        header('Location:request made.php');
    } else {
        echo "Error updating request: " . htmlspecialchars($stmt->error);
    }
    $stmt->close();
    exit;
}
...
<head>
    ...
    <title>Customer Request Page</title>
...
<body>
    ...
    <div class="gradient-background">
        <center>
        <div  style="margin-top:10%;width:650px;background-color:rgba(0, 0, 0, 0.484);border-radius:10px;" >
            <br>
            <div style="font-size: 45px;margin-bottom: 20px;margin-top: 10px;color:white">
                FEEDBACK
            </div>
            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . '?req_id=' . urlencode($req_id); ?>">
                
                <div class="form-floating mb-3" style="width:550px;">
                    <input name="v_type" type="text" class="form-control" id="floatingInput" placeholder="" required>
                    <label for="floatingInput">Customer name</label>
                </div>

                <div class="form-floating" style="width:550px">
                    <textarea name="request" type="text" class="form-control" placeholder="Leave a comment here" id="floatingTextarea2" style="height: 100px;margin-bottom:3%"></textarea>
                    <label for="floatingTextarea2">Request</label>
                    </div>

                <br><button style="height:40px;
                width:550px;
                background-color: #000000;
                color:white;
                border-radius:10px;font-size:17px;
                border-color: #000000 ;
                margin-bottom: 6%;">SUBMIT</button>
            </form>
